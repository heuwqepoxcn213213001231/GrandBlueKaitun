...
def main() -> int:
    ...
    if "function GB.SelfCheck" not in read("src/boot.lua"):
        fail("SelfCheck missing")
    if "function GB.SelfCheck" not in read("kaitun.lua"):
        fail("generated kaitun.lua missing SelfCheck")
    if "GENERATED by tools/build_single.py" not in read("kaitun.lua"):
